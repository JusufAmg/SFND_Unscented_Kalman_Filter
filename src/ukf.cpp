...
UKF::UKF() {
 // if this is false, laser measurements will be ignored (except during init)
  ...
  Tc_lidar_ =  MatrixXd(n_x_, n_z_lidar_).setZero();
  Tc_radar_ =  MatrixXd(n_x_, n_z_radar_).setZero();
}
...
void UKF::Prediction(double delta_t) {
  ...
  MatrixXd P_aug_ = MatrixXd(7, 7).setZero();
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
   ...
   */
	MatrixXd Zsig = MatrixXd(n_z_lidar_, 2 * n_aug_ + 1).setZero();
	...
	VectorXd z_pred = VectorXd(n_z_lidar_).setZero();
    ...
    MatrixXd S = MatrixXd(n_z_lidar_,n_z_lidar_).setZero();
  ...
  // Kalman gain K;
  MatrixXd K = MatrixXd(n_x_, n_z_lidar_).setZero();
